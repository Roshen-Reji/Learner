...
export default function RoadmapPage() {
  ...
  if (!selectedRoadmap) {
    return (
      <div className="min-h-screen bg-surface-light">
        <Sidebar />
        <main className="lg:ml-72 pt-20 lg:pt-8 pb-28 lg:pb-8 min-h-screen flex flex-col">
          <div className="px-4 sm:px-8 shrink-0">
            <h1 className="text-2xl sm:text-4xl font-black text-text-primary flex items-center gap-3 drop-shadow-sm">
              <Map className="text-primary" size={32} /> Master Your Craft
            </h1>
            <p className="text-text-secondary mt-1 font-medium">Swipe to explore curated roadmaps</p>
          </div>
          ...
          <div className="w-full mt-6 md:mt-10 flex flex-col items-center">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center flex-1 h-full gap-4">
                ...
                <p className="text-text-secondary font-medium">Loading roadmaps...</p>
              </div>
            ) : roadmaps.length === 0 ? (
              <div className="flex flex-col items-center justify-center w-full max-w-sm text-center mt-20">
                <Map className="text-text-secondary/50 mb-4" size={64} />
                <h3 className="text-xl font-bold text-text-primary">No Roadmaps Found</h3>
                <p className="text-text-secondary mt-2">Generate your first AI roadmap above!</p>
              </div>
            ) : (
              <div className="w-full overflow-x-auto snap-x snap-mandatory flex gap-5 px-4 sm:px-8 pb-10 pt-4 custom-scrollbar items-center">
                {/* 
                ...
                */}
                {roadmaps.map((rm) => (
                  <button
                    ...
                    className="group relative w-[85vw] sm:w-[360px] lg:w-[400px] h-[65vh] min-h-[450px] max-h-[600px] shrink-0 snap-center rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-[0_15px_40px_-15px_rgba(0,0,0,0.3)] hover:shadow-[0_25px_50px_-12px_rgba(0,120,255,0.4)] transition-all duration-500 scale-95 hover:scale-100 text-left border border-white/20 dark:border-white/5"
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 top-1/2 bg-gradient-to-t from-black/95 via-black/60 to-transparent transition-opacity duration-300 group-hover:opacity-100 opacity-90" />
                    <div className="absolute inset-0 bg-primary/10 mix-blend-overlay group-hover:bg-transparent transition-colors duration-500" />
                    
                    {/* Private Badge */}
                    {rm.approved === false && (
                       <div className="absolute top-6 left-6 bg-black/50 backdrop-blur-md text-amber-400 border border-amber-500/30 px-3 py-1.5 rounded-full text-xs font-black uppercase tracking-widest flex items-center gap-1.5 shadow-lg">
                         <Lock size={12} /> Private
                       </div>
                    )}

                    {/* Content */}
                    <div className="absolute inset-x-0 bottom-0 p-8 flex flex-col justify-end transform transition-transform duration-500 group-hover:translate-y-[-8px]">
                      ...
                      <p className="text-white/70 text-sm line-clamp-2 leading-relaxed drop-shadow-sm font-medium">
                        {rm.description}
                      </p>

                      <div className="mt-6 flex items-center text-blue-300 font-bold gap-2 text-sm opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-[-10px] group-hover:translate-x-0">
                        Start Journey <MoveRight size={18} className="group-hover:translate-x-1 transition-transform" />
                      </div>
                    </div>
                  </button>
                ))}
  ...
  return (
    <div className="min-h-screen relative bg-black">
      ...
      <Sidebar />
      <main className="relative z-10 lg:ml-72 pt-20 lg:pt-8 pb-28 lg:pb-8 min-h-screen">
        <div className="max-w-3xl mx-auto px-4 sm:px-8 pb-12 relative z-10">
          
          <button
            onClick={() => { setSelectedRoadmap(null); setActiveNode(null); }}
            className="flex items-center gap-2 bg-white/60 dark:bg-black/40 backdrop-blur-lg border border-white/50 dark:border-white/10 text-text-primary px-4 py-2 rounded-full hover:bg-white dark:hover:bg-slate-800 transition-all shadow-sm mb-8 font-semibold text-sm group w-fit"
          >
          ...
          {/* Header Card */}
          <div className="bg-white/40 dark:bg-slate-900/50 backdrop-blur-3xl border border-white/60 dark:border-white/10 p-6 sm:p-8 rounded-[2rem] shadow-2xl mb-12">
            <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between mb-4">
              <div className="flex items-center gap-4">
                <div className="text-4xl w-16 h-16 rounded-2xl bg-white/60 dark:bg-white/10 flex items-center justify-center shadow-inner border border-white/40 dark:border-white/5">
                  {selectedRoadmap.icon}
                </div>
                <div>
                  <h1 className="text-3xl sm:text-4xl font-extrabold text-text-primary flex items-center gap-3 tracking-tight">
                    {selectedRoadmap.skill}
                  </h1>
                  <div className="flex items-center gap-2 mt-2">
                    {selectedRoadmap.approved === false && (
                      <span className="text-[10px] px-2.5 py-1 rounded-full bg-amber-500 text-white uppercase font-black tracking-widest shadow-md">
                        Private
                      </span>
                    )}
                    <span className="text-sm font-bold text-primary flex items-center gap-1.5"><Layers size={14}/> {selectedRoadmap.nodes.length} Stages</span>
                  </div>
                </div>
              </div>
            </div>
            
            <p className="text-text-secondary font-medium leading-relaxed max-w-2xl text-sm sm:text-base">
          ...
          {/* Glassy Timeline */}
          <div className="space-y-6 relative before:absolute before:inset-y-0 before:left-8 sm:before:left-10 before:w-1 before:bg-gradient-to-b before:from-primary/30 before:via-secondary/30 before:to-transparent before:rounded-full before:shadow-[0_0_15px_rgba(59,130,246,0.4)]">
            {selectedRoadmap.nodes
              ...
              .map((node, i) => {
                ...
                return (
                  <div key={i} className="relative z-10 pl-20 sm:pl-24">
                    {/* Node Dot / Checked Circle floating directly over the vertical bar */}
                    <div 
                      className={`absolute left-4 sm:left-6 top-6 w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-bold text-sm shadow-xl transition-all duration-500 z-20 ${
                        completed 
                          ? "bg-gradient-to-br from-green-400 to-emerald-600 text-white shadow-[0_0_20px_rgba(16,185,129,0.5)] scale-110" 
                          : isActive
                            ? "bg-primary text-white shadow-[0_0_20px_rgba(59,130,246,0.6)] scale-125 ring-4 ring-primary/20"
                            : "bg-white dark:bg-slate-800 text-text-secondary border-2 border-border dark:border-white/10"
                      }`}
                    >
                      {completed ? <Check size={20} className="drop-shadow-sm" /> : <span>{i + 1}</span>}
                    </div>

                    <div 
                      className={`bg-white/50 dark:bg-slate-900/60 backdrop-blur-2xl border border-white/60 dark:border-white/10 rounded-[2rem] transition-all duration-500 overflow-hidden shadow-lg hover:shadow-xl ${
                    ...
                    >
                      <button
                        ...
                        className="w-full text-left p-5 sm:p-6 flex items-start sm:items-center justify-between gap-4 outline-none"
                      >
                        <div>
                          <h3 className={`font-bold text-lg sm:text-xl transition-colors ${completed ? "text-emerald-600 dark:text-emerald-400" : isActive ? "text-primary dark:text-blue-400" : "text-text-primary"}`}>
                            {node.title}
                          </h3>
                          <p className={`text-sm mt-1 sm:mt-1.5 transition-all ${isActive ? "text-text-primary font-medium" : "text-text-secondary line-clamp-2"}`}>
                            {node.description}
                          </p>
                        </div>
